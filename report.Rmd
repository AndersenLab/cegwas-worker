...
if (length(commandArgs(trailingOnly=TRUE)) == 0) {
  args <- fromJSON('{"version": 0.1, "email": "danielecook@gmail.com", "release": 0, "report_slug": "great", "trait_name": "telomere_length", "report_name": "great"}')
} else {
...
trait <- collect(tbl(db, "report_trait_strain") %>%
             ...
             dplyr::filter(name == args$trait_name, report_slug == args$report_slug) %>%
...
tryCatch(
proc_mappings <- process_mappings(mapping,trait) %>% dplyr::filter(log10p !=0) %>% filter(!(grepl("MtDNA",marker) & log10p < BF)),
...
`r if (nrow(proc_mappings) == 0) '# Reducing Signficance Threshold'`
